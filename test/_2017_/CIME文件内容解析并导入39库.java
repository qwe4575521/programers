...
public class CIME文件内容解析并导入39库 {
    public static void main(String[] args) throws IOException {
        ...
        String[] files = new String[]{"jiangxi_20180112_150335.CIME"};
    ...
    public static String getTablename(String strTemp, String Tablename) {
        ...
        if (strTemp.contains("<") && !strTemp.contains("<!")) {
            String[] tablenames = new String[]{"Breaker", "Disconnector", "Disconnector", "BusbarSection", "SynchronousMachine", "Load", "ACLineSegment", "ACLineDot", "PowerTransformer", "TransformerWinding", "ShuntCompensator"};
    ...
    public static void setInsertSql(String Tablename, String strTemp, JdbcTemplate jdbcTemplate) {
        ...
        List<Map<String, Object>> list = jdbcTemplate.queryForList(sql, new Object[]{Tablename});
        if (strTemp.length() != 0) {
            //找出表及字段内容
            if (strTemp.contains("#")) {
                strTemp = strTemp.replace("\t ", "\t").replace("    ", "\t").replace("   ", "\t").replace("  ", "\t").replace(" ", "\t");
                String[] split = strTemp.split("\t");
                StringBuffer result = new StringBuffer().append(" insert into HD_DB.CIME." + Tablename + " (");
